...
function saveVoiceId()
{
    ...
    var formData = $('#saveVoiceId').serialize();
    $.ajax({'url': base_url +  'user/save_voice_id', type: 'POST', data: formData, 'success': function (data) {
            $btn.button('reset')
            var container = $('#FileDiv2');
...
$("#upload_audio_file").change(function (e) {
    ...
    if ($.inArray(ext, ["mp3", "wav"]) === -1) {
     ...
     $('#loading_text').removeClass('myhidden');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response) {
                   $('#loading_text').addClass('myhidden');
                container.html(response);
                var msg_type = $('#msg_type').val();
                var msg_data = $('#msg_data').val();
                if (msg_type === '1') {
                    ...
                    $('span#notification').html(msg_data);
                    $("input#upload_audio_file").val(null);
                    $('#my_div').fadeOut();   //for loader hidr this line call when msg 1 genrate
...
$("#upload_attach_file").change(function (e) {
   
    $('span#notification').removeClass("notification").removeClass("alert-success").removeClass("alert-danger").removeClass("alert-warning").html("");
    var ext = $("input#upload_attach_file").val().split(".").pop().toLowerCase();
   
    
     $('#loading_text').removeClass('myhidden');
    var file_data = $('#upload_attach_file').prop('files')[0];
    var form_data = new FormData();
    form_data.append('upload_attach_file', file_data);
    var base_url = $('#base_url').val();
    
    var url = base_url + '/user/upload_attach_file';
    $.ajax({
        url: url,
        type: 'POST',
        data: form_data,
        ...
        success: function (response) {
            ...
            if (response) {
                ...
                container.html(response);
                var msg_type = "1";
                var msg_data = "file uploaded successfully";
               
                if (msg_type === '1') {
                    $('span#notification').addClass("notification alert-success");
                    $('span#notification').html(msg_data);
                    $("input#upload_attach_file").val(null);
                    $('#my_div').fadeOut();   //for loader hidr this line call when msg 1 genrate
                ...
                $('span#notification').addClass("notification alert-danger");
                $('span#notification').html("<i class='fa fa-exclamation-circle'></i> Error: Please upload valid FILE");
                $("input#upload_attach_file").val(null);
                return false;
            }
        }
