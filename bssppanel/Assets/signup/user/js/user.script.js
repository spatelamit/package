...
function checkKeywordAvailability(type, keyword)
{
    ...
    } else if (type === 'long')
    {
     ...
     /*   if (number.length < 10) {
        ...
        if (keyword.length < 6) {
            $('span#notification').addClass("notification alert-danger");
            $('span#notification').html('<i class="fa fa-exclamation-circle"></i> Error: Keyword must be of 6 characters');
            return false;
        }
    }
...
$(document).ready(function ()
{
    ...
    $("form#sendVoiceSMSForms").validate({rules: {"campaign_name": {required: true}, "caller_id": {required: true, minlength: 10, maxlength: 10}, "duration": {required: true}}, messages: {"campaign_name": {required: "Please enter campaign name"}, "caller_id": {required: 'Please enter caller id', minlength: 'Caller_id id must be 10 digits long', maxlength: 'Caller_id id must be 10 digits long'}, "duration": {required: 'Please enter duration'}}, submitHandler: function () {
            $('span#notification').html("");
            $('span#notification').removeClass("notification alert-success alert-danger alert-warning");
            var mobile = $('#mobile_numbers').val();
            var csv = $('#mobiles').val();
