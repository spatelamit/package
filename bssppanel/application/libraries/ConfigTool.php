        ...
        if (isset($this->$pKey)) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if (preg_match("/^[A-Z]/i", $ar[$i])) {
                    list ( $key, $val ) = preg_split("/=/i", $ar[$i]);
                    $key = preg_replace("/[ 	]/i", "", $key);
                    if ($key == $pKey) {
                        $add = false;
                    } else
...
//END OF FUNCTION _arrayToString
    ...
    function _fileWrite($pFlag, &$pContent) {
        ...
        /*
          ...
          if (fwrite($fp, ($pFlag == 'a') ? "\n\n" . $pContent : $pContent) === FALSE) {
