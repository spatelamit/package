...
if (empty($XMLFUNC_PHP)) {
    ...
    function smsc_details($inst, $xml) {
        ...
        while (($y = XPathValue("smsc", $x)) != "") {
            ...
            switch ($a[0]) {
                case "online":
                    echo "<span class='label label-success'>online</span> <br />";
                    echo " (" . $z . "s)";
                    break;
                case "disconnected":
                    ...
                    break;
                case "connecting":
                    echo "<span class='label label-danger'>connecting</span>";
                    break;
                case "re-connecting":
                    echo "<span class='label label-danger'>re-connecting</span>";
                    break;
                case "dead":
            ...
            echo "</td><td valign=top class=text nowrap>\n";
            if (preg_match("/online (.*)s/i", XPathValue("status", $y), $regs)) {
                $z = $regs[1];
            ...
            echo "</td><td class='text-center button-group' nowrap>\n";
            echo "<a class='href btn btn-success btn-xs' href=\"#\" onClick=\"admin_smsc_url('start-smsc', '";
            echo $config["base_url"] . "/start-smsc?smsc=$smsc','";
            echo "$smsc', '" . $config["admin_passwd"] . "');\">start</a>&nbsp;&nbsp;";
            echo "<a class='href btn btn-danger btn-xs' href=\"#\" onClick=\"admin_smsc_url('stop-smsc', '";
            echo $config["base_url"] . "/stop-smsc?smsc=$smsc','";
            echo "$smsc', '" . $config["admin_passwd"] . "');\">stop</a>";
            echo "</td></tr>\n";
            $a = substr($x, strpos($x, "</smsc>") + 7);
            $x = $a;
        }
